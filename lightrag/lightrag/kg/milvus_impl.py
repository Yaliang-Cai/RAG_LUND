...
class MilvusVectorDBStorage(BaseVectorStorage):
    ...
    def _uri_is_remote(uri: str) -> bool:
        """Return True when the URI points to a remote Milvus server (not milvus-lite)."""
        return uri.startswith(("http", "https", "tcp", "unix"))
    ...
    def _get_milvus_connection_kwargs(self, include_db_name: bool = True) -> dict:
        ...
        For milvus-lite (local .db file): only 'uri' is included — credentials
        and db_name are silently dropped because milvus-lite does not support them.
        """
        uri = os.environ.get(
            "MILVUS_URI",
            config.get(
        ...
        connection_kwargs: dict = {"uri": uri}

        # Credentials and db_name are only meaningful for remote HTTP servers.
        if self._uri_is_remote(uri):
            user = os.environ.get(
    ...
    def _get_milvus_db_name(self) -> Optional[str]:
        """Return the configured Milvus database name, if any (remote mode only)."""
    ...
    def _create_milvus_client(self) -> MilvusClient:
        ...
        For milvus-lite: simply return the client; database creation / switching
        is not supported and will raise an error if attempted.
        """
        ...
        if not self._uri_is_remote(uri):
            logger.info(
                f"[{self.workspace}] Using milvus-lite at {uri}"
            )
            return client
    ...
    def _create_schema_for_namespace(self) -> CollectionSchema:
        ...
        # Use plain NOT NULL VARCHAR for lite; upsert will fill "" when absent.
        is_lite = self._is_milvus_lite()

        def varchar_field(name: str, max_length: int) -> FieldSchema:
            kwargs: dict = {
                "name": name,
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        # milvus-lite fields are NOT NULL — fill missing meta_fields with "".
        # Full Milvus fields are nullable — missing fields can be omitted.
        _lite = self._is_milvus_lite()

        def _row(k: str, v: dict) -> dict:
            row = {
                "id": k,
