...
### 当前状态
- 4 个文件已实现，待 commit push。

---

## 增量更新（2026-03-23，DocBench Shared Ingest 并行化）

### 背景
- 目标：`evaluate_shared.py` 在共享图谱（同一个 `shared_doc_id`）场景下支持“多文档并行 ingest”。
- 旧行为：虽然可在脚本层写并发，但 `LocalRagService.ingest()` 对同 `doc_id` 有串行锁，导致共享图谱场景最终仍串行。

### 改动 1：`evaluate_shared.py` 增加 ingest 并发参数并改为批量并发

- 新增参数：`--max_async_ingest`（默认 `4`，最小归一化为 `1`）。
- 生成阶段（Phase 1）改为：
  - 先收集待 ingest 文档列表；
  - 按 `max_async_ingest` 分批；
  - 每批使用 `asyncio.gather(..., return_exceptions=True)` 并发执行。
- 成功后更新 manifest；批次内任一失败则 fail-fast 抛错（避免静默跳过）。
- `generation_config.json` 新增 `max_async_ingest` 记录字段。

### 改动 2：`local_rag.py` 增加可选的同 `doc_id` 串行锁开关

- `LocalRagService.ingest()` 新增参数：
  - `serialize_by_doc_id: bool = True`（默认保持旧行为，不影响现有调用）。
- 当 `serialize_by_doc_id=False` 时，跳过同 `doc_id` 锁，仅用于 shared 并发 ingest 场景。
- `evaluate_shared.py` 并发 ingest 时显式传 `serialize_by_doc_id=False`。

### 兼容性与行为边界

- 默认行为不变：
  - `evaluate.py`、其他现有调用未传新参数，仍按原串行锁逻辑执行。
- 仅 shared 并发场景变更：
  - 同一 `shared_doc_id` 下，脚本层并发现在可真正生效。
- 参数边界：
  - `max_async_ingest<=0` 会被归一到 `1`。

### 变更文件

| 文件 | 改动 |
|------|------|
| `rag-anything/evaluate_local/DocBench/evaluate_shared.py` | 新增 `--max_async_ingest`；Phase 1 改为分批并发 ingest；配置落盘新增字段 |
| `rag-anything/raganything/services/local_rag.py` | `ingest()` 新增 `serialize_by_doc_id` 开关（默认 `True`） |
