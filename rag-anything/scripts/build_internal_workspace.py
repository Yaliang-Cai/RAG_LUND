...
def _manual_convert_command(source_path: Path, pdf_path: Path) -> str:
    ...
    return "\n".join(
        [
            ...
            'log_dir="${manual_convert_log_dir:-$out/manual_convert_logs}"',
            'log_file="$log_dir/$(basename "$src").log"',
            'IFS=" " read -r -a libreoffice_commands <<< "${LIBREOFFICE_CONVERT_COMMANDS:-libreoffice soffice}"',
            'mkdir -p "$out" "$log_dir"',
            ': > "$log_file"',
            ...
            'status=0',
            'for lo_cmd in "${libreoffice_commands[@]}"; do',
            '  [ -n "$lo_cmd" ] || continue',
            '  rm -rf "$profile"',
            '  profile="$(mktemp -d /tmp/lo_profile_XXXXXX)"',
            '  echo "COMMAND: $lo_cmd -env:UserInstallation=file://${profile} --headless --convert-to pdf --outdir $out $src" >> "$log_file"',
            '  timeout "${LIBREOFFICE_CONVERT_TIMEOUT_SECONDS:-60}" \\',
            '    "$lo_cmd" "-env:UserInstallation=file://${profile}" \\',
            '    --headless --convert-to pdf --outdir "$out" "$src" >> "$log_file" 2>&1 || status=$?',
            f"  if [ -s {_shell_quote(pdf_path)} ]; then",
            "    break",
            "  fi",
            ...
            'rm -rf "$profile"',
            f"if [ -s {_shell_quote(pdf_path)} ]; then",
            f"  touch {_shell_quote(pdf_path)}",
            "  echo \"OK: $src\"",
            "elif [ \"$status\" -ne 0 ]; then",
            "  echo \"FAILED: $src (see $log_file)\" >&2",
            "  manual_convert_failed=1",
            "else",
            "  echo \"FAILED: expected PDF was not created for $src (see $log_file)\" >&2",
            "  manual_convert_failed=1",
            "fi",
        ]
    )
