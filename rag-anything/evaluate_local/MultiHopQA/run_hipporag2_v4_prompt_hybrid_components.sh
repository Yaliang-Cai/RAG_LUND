...
# MultiHopQA V4 PPR hybrid-component runner with semantic passage CoT QA prompt.
#
# This runner reuses existing V0 HippoRAG2 MultiHopQA workspaces. It does not
# download data, build indexes, or rebuild SYNONYM edges. By default it skips
# the already-run hybrid anchor and only evaluates the three additional
# hybrid-based component combinations.
...
RESULTS_ROOT="${RESULTS_ROOT:-${RAGANYTHING_ROOT}/evaluate_local/MultiHopQA/results/multihopqa_hr2_v0_v4_components_semantic_prompt_hybrid}"
...
RUN_HYBRID_ANCHOR="${RUN_HYBRID_ANCHOR:-0}"
...
EXPERIMENTS=(
    "ppr_hybrid_plus_per_keyword"
    "ppr_hybrid_plus_synonym"
    "ppr_hybrid_plus_per_keyword_plus_synonym"
)
if [[ "${RUN_HYBRID_ANCHOR}" == "1" ]]; then
    EXPERIMENTS=("ppr_hybrid_anchor" "${EXPERIMENTS[@]}")
fi
...
experiment_config() {
    ...
    case "${experiment}" in
        ppr_hybrid_anchor)
            printf '%s\t%s\t%s\n' "joined" "hybrid" "--exclude-synonym-edges"
            ;;
        ppr_hybrid_plus_per_keyword)
            printf '%s\t%s\t%s\n' "per_keyword_rrf" "hybrid" "--exclude-synonym-edges"
            ;;
        ppr_hybrid_plus_synonym)
            printf '%s\t%s\t%s\n' "joined" "hybrid" "--no-exclude-synonym-edges"
            ;;
        ppr_hybrid_plus_per_keyword_plus_synonym)
            printf '%s\t%s\t%s\n' "per_keyword_rrf" "hybrid" "--no-exclude-synonym-edges"
            ;;
        *)
...
run_ppr_experiment() {
    ...
    local keyword_fanout_mode=""
    local qdrant_retrieval_mode=""
    local synonym_flag=""
    local output_dir="${RESULTS_ROOT}/${dataset}/${experiment}"
    local eval_resume_arg=()

    IFS=$'\t' read -r keyword_fanout_mode qdrant_retrieval_mode synonym_flag < <(experiment_config "${experiment}")
    if [[ "${EVAL_RESUME}" == "1" ]]; then
        eval_resume_arg=(--resume)
    fi
    if [[ "${synonym_flag}" == "--no-exclude-synonym-edges" ]]; then
...
print_summary() {
    python - "$RESULTS_ROOT" "${EXPERIMENTS[@]}" -- "${DATASETS[@]}" <<'PY'
import json
import sys
from pathlib import Path
...
log "================================================================"
log "MultiHopQA V4 PPR hybrid components with semantic passage CoT QA prompt"
...
log "Run hybrid anchor:  ${RUN_HYBRID_ANCHOR}"
...
for DATASET in "${DATASETS[@]}"; do
    ...
    for EXPERIMENT in "${EXPERIMENTS[@]}"; do
        run_ppr_experiment "${EXPERIMENT}" "${DATASET}" "${WORKING_DIR}" "${WORKSPACE_ID}"
    done
done
