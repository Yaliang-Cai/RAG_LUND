...
# MultiHopQA marginal window/filter sensitivity runner.
#
# This script reuses the existing V0 MultiHop workspaces. It does not build an
# index, does not rebuild SYNONYM edges, and only runs the added experiments.
# It only runs the additional single-variable experiments listed below.
#
# Expected prior state:
# - Workspaces live under WORKSPACE_ROOT and were built with profile v0.
# - SYNONYM edges at threshold 0.8 were already applied by the synonym ablation
#   runner. PPR additions below intentionally use those edges.
...
RESULTS_ROOT="${RESULTS_ROOT:-${RAGANYTHING_ROOT}/evaluate_local/MultiHopQA/results/multihopqa_hr2_v0_window_sensitivity}"
...
run_eval() {
    ...
    local chunk_top_k="$7"
    local ppr_top_k="$8"
    local ppr_qa_top_k="$9"
    local min_rerank_score="${10}"
    shift 10
    ...
    mkdir -p "${output_dir}"
    log "[${dataset}] ${experiment}: mode=${mode}, top_k=${top_k}, chunk_top_k=${chunk_top_k}, ppr_top_k=${ppr_top_k}, ppr_qa_top_k=${ppr_qa_top_k}, min_rerank_score=${min_rerank_score}"
...
run_non_ppr_experiment() {
    ...
    run_eval \
        ...
        --keyword-fanout-mode "per_keyword_rrf" \
...
run_ppr_experiment() {
    ...
    local top_k="$2"
    local ppr_top_k="$3"
    local ppr_qa_top_k="$4"
    local min_rerank_score="$5"
...
experiments = [
    "non_ppr_top20_chunk5",
    "non_ppr_top30_chunk5",
    "non_ppr_top10_chunk10",
    "non_ppr_top10_chunk5_min0p0",
    "ppr_top20_ppr50_qa5",
    "ppr_top30_ppr50_qa5",
    "ppr_top10_ppr100_qa5",
    "ppr_top10_ppr50_qa10",
    "ppr_top10_ppr50_qa5_min0p0",
]

print("dataset\texperiment\tmode\tem\tf1\trecall@2\trecall@5")
for dataset in datasets:
    for experiment in experiments:
        path = results_root / dataset / experiment / f"{dataset}_summary.json"
        if not path.exists():
...
log "================================================================"
log "MultiHopQA window/filter sensitivity"
